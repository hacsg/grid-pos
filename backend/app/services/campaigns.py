...
async def list_customers(db: AsyncSession) -> list[Customer]:
    """Return customers with signup campaign info and voucher counts."""
    voucher_counts = (
        select(
            Voucher.customer_id,
            func.count(Voucher.id).label("voucher_count"),
        )
        .where(Voucher.customer_id.is_not(None))
        .group_by(Voucher.customer_id)
        .subquery()
    )
    result = await db.execute(
        select(
            Customer,
            func.coalesce(voucher_counts.c.voucher_count, 0).label("voucher_count"),
        )
        .outerjoin(voucher_counts, Customer.id == voucher_counts.c.customer_id)
        .options(selectinload(Customer.signup_campaign))
        .order_by(Customer.name.asc())
    )
    customers: list[Customer] = []
    for customer, voucher_count in result.all():
        customer.voucher_count = int(voucher_count or 0)  # type: ignore[attr-defined]
